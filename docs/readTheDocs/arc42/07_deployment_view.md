...
- All services use `restart: unless-stopped`. The `./data` directory is gitignored and created on
  first run.
- Under Linux: Depending on the host's settings, the `./data` directory is owned by `root`. In this case, the ownership of the directory must first be changed to your user before a new user can be added.
